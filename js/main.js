// Main JavaScript for Bintang Bangsa Website
// Following modern ES6+ practices and accessibility guidelines

class BintangBangsaWebsite {
    constructor() {
        this.init();
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = BintangBangsaWebsite;
} 
...
// Instansi Carousel Functionality
document.addEventListener('DOMContentLoaded', function() {
  const instansiCards = document.querySelectorAll('.instansi-card');
  const instansiDots = document.querySelectorAll('.instansi-dot');
  const prevBtn = document.querySelector('.instansi-prev-btn');
  const nextBtn = document.querySelector('.instansi-next-btn');
  
  let currentIndex = 0;
  const cardsPerView = 2;
  const totalSlides = Math.ceil(instansiCards.length / cardsPerView);
  ...
  function showCards(startIndex) {
    ...
    instansiDots.forEach(dot => {
      dot.classList.remove('active');
    });
    
    // Show current 2 cards
    for (let i = 0; i < cardsPerView; i++) {
      const cardIndex = startIndex * cardsPerView + i;
      if (instansiCards[cardIndex]) {
        instansiCards[cardIndex].style.display = 'block';
      }
    }
    
    // Show current dot
    if (instansiDots[startIndex]) {
      instansiDots[startIndex].classList.add('active');
    }
    
    // Update button states
    prevBtn.style.opacity = startIndex === 0 ? '0.5' : '1';
    nextBtn.style.opacity = startIndex >= totalSlides - 1 ? '0.5' : '1';
  }
  ...
  // Dot navigation
  instansiDots.forEach((dot, index) => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Function to get total slides based on current cards per view
  ...
  function showTestimonials(startIndex) {
    ...
    // Calculate the transform value to slide the container
    // Each slide should move by 50% (half the container width)
    const transformValue = -(startIndex / cardsPerView) * 50;
    ...
    // Show current dot
    const currentDotIndex = Math.floor(startIndex / cardsPerView);
    if (testimonialDots[currentDotIndex]) {
      testimonialDots[currentDotIndex].classList.add('active');
    }
    
    // Update button states
    prevBtn.disabled = startIndex === 0;
    nextBtn.disabled = startIndex >= testimonialCards.length - cardsPerView;
...
// Remove testimonial carousel functionality since we're using grid layout 